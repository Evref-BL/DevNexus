...
try {
  ...
  console.error("DevNexus Claude wrapper could not load the local DevNexus build.");
...
const plan = api.planNexusAgentClientAdapterCommand({
  ...
  pluginDataRoot: process.env.CLAUDE_PLUGIN_DATA,
  env: process.env,
});
...
await new Promise((resolve) => {
  child.on("exit", (code, signal) => {
    if (signal) {
      console.error(`DevNexus Claude wrapper child exited from signal ${signal}.`);
      process.exitCode = 1;
      resolve();
      return;
    }
  ...
  child.on("error", (error) => {
    console.error(`DevNexus Claude wrapper failed: ${error.message}`);
    process.exitCode = 1;
    resolve();
  });
});
