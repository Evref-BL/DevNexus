...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "  const plugins = loading && !pluginsLoaded ? renderProgressivePanel('plugins-panel', 'Extensions', 'Plugins', 'Loading curated plugin catalogue and capability details.') : renderPlugins(snapshot.plugins);",
    ...
    "  const available = plugins?.availableCount ?? records.filter((plugin) => plugin.source === 'catalogue' || plugin.state === 'available').length;",
    "  const disabled = records.filter((plugin) => plugin.source !== 'catalogue' && !plugin.enabled).length;",
    ...
    "    if (!plugin.refreshCommand) return `<div class=\"dn-action-strip compact\">${renderDisabledAction('Refresh unavailable', 'No curated plugin refresh command is available', signalIcon('plugins'))}</div>`;",
...
export function auditNexusDashboardClientVisuals(
  ...
  const checks = [
    ...
    visualAuditCheck(
      ...
      "Configured and catalogue plugins have their own compact cockpit section.",
    ),
    visualAuditCheck(
      "tracked-work",
      "Tracked work cards",
