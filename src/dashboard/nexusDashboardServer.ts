...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "        addGitHistoryLaneTransition(route, existingLane, index + 0.85);",
    ...
    "      if (preferredLane !== lane) addGitHistoryLaneTransition(route, preferredLane, index + 0.85);",
    "      active[preferredLane] = route;",
    "    }",
    "    compactGitHistoryActiveLanes(active, index);",
    "  }",
    "  const parentPaths = gitHistoryParentPaths(rows);",
    "  const maxLane = Math.max(0, ...rows.map((row) => row.lane), ...parentPaths.flatMap((path) => path.points.map((point) => point.lane)));",
    "  return { repository, rows, paths: parentPaths, maxLane };",
    "}",
    "",
    "function gitHistoryParentPaths(rows) {",
    "  const rowsByHash = new Map(rows.map((row) => [row.commit.hash, row]));",
    "  const paths = [];",
    "  for (const row of rows) {",
    "    const parents = row.commit.parents ?? [];",
    "    for (let parentIndex = 0; parentIndex < parents.length; parentIndex++) {",
    "      const parentRow = rowsByHash.get(parents[parentIndex]);",
    "      if (!parentRow) continue;",
    "      const colorLane = parentIndex === 0 ? row.colorLane ?? row.lane : parentRow.colorLane ?? parentRow.lane;",
    "      paths.push({ colorLane, fromLane: row.lane, toLane: parentRow.lane, fromIndex: row.index, toIndex: parentRow.index, points: gitHistoryParentPathPoints(row, parentRow) });",
    "    }",
    "  }",
    "  return paths;",
    "}",
    "",
    "function gitHistoryParentPathPoints(row, parentRow) {",
    "  const start = { lane: row.lane, index: row.index };",
    "  const end = { lane: parentRow.lane, index: parentRow.index };",
    "  if (start.lane === end.lane) return [start, end];",
    "  const corridorIndex = Math.max(start.index + 0.72, end.index - 0.38);",
    "  return [start, { lane: start.lane, index: corridorIndex }, { lane: end.lane, index: corridorIndex }, end];",
    "}",
    "",
    ...
    "}",
    "",
    "function addGitHistoryLaneTransition(route, lane, index) {",
    "  if (!route) return;",
    "  const last = route.points[route.points.length - 1];",
    "  if (!last || last.lane === lane) {",
    "    addGitHistoryRoutePoint(route, lane, index);",
    "    return;",
    "  }",
    "  const anchorIndex = Math.max(last.index, index - 0.4);",
    "  addGitHistoryRoutePoint(route, last.lane, anchorIndex);",
    "  addGitHistoryRoutePoint(route, lane, index);",
    "}",
    "",
    ...
    "      addGitHistoryLaneTransition(entry, write, rowIndex + 0.95);",
    "      active[write] = entry;",
    "      active[read] = null;",
    "    }",
    "    write++;",
