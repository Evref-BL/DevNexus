...
export async function dashboardPortInUseMessage(options: {
  ...
  const intro = `Dashboard port ${options.host}:${options.port} is already in use.`;
  ...
  const knownSentence = known
    ? ` Known DevNexus dashboard: ${known.url}.`
    : "";
  const scopedStatus = projectRoot
    ? `dev-nexus dashboard status ${projectRoot}`
    : "dev-nexus dashboard status";
  const scopedServe = projectRoot
    ? `dev-nexus dashboard serve ${projectRoot}`
    : "dev-nexus dashboard serve";
  return [
    `${intro}${knownSentence}`,
    `Run ${scopedStatus} to inspect known dashboards, ${scopedServe} --host ${options.host} --port ${options.port} --restart to replace a verified DevNexus dashboard, or choose a different --port.`,
  ].join(" ");
}
