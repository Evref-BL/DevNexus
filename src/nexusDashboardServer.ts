...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ".dn-signal { position: relative; min-height: 136px; overflow: hidden; padding: 14px; text-align: left; color: inherit; cursor: pointer; transition: transform 160ms ease, border-color 160ms ease, background 160ms ease; }",
    ...
    ".dn-signal:hover, .dn-component-card:hover, .dn-event:hover, .dn-blocker:hover, .dn-history-item:hover { transform: translateY(-1px); }",
    ...
    ".dn-signal-top, .dn-card-title, .dn-history-title, .dn-panel-heading { display: flex; align-items: center; justify-content: space-between; gap: 10px; min-width: 0; }",
    ...
    ".dn-signal p { display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }",
    ...
    ".dn-history-panel { min-height: 620px; background: linear-gradient(180deg, color-mix(in srgb, var(--dn-surface) 96%, var(--dn-branch-0) 4%), var(--dn-surface)); }",
    ".dn-count { color: var(--dn-label); font-size: 0.8rem; font-weight: 800; white-space: nowrap; }",
    ".dn-branch-board { position: relative; min-height: 420px; margin-top: 12px; padding-left: 132px; overflow: hidden; }",
    ...
    ".dn-history-item { position: relative; display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 14px; min-height: 72px; padding: 12px 12px 12px 16px; border: 1px solid transparent; border-left: 3px solid var(--dn-branch-color); border-radius: 8px; color: inherit; background: transparent; text-align: left; cursor: pointer; transition: transform 160ms ease, background 160ms ease, border-color 160ms ease; }",
    ".dn-history-item:hover { background: color-mix(in srgb, var(--dn-control-hover) 82%, var(--dn-branch-color) 18%); }",
    ".dn-branch-dot { position: absolute; left: calc(-110px + (var(--dn-lane) * 18px)); top: calc(50% - 8px); width: 16px; height: 16px; border: 3px solid var(--dn-surface); border-radius: 999px; background: var(--dn-branch-color); box-shadow: 0 0 0 2px var(--dn-branch-color), 0 0 18px color-mix(in srgb, var(--dn-branch-color) 62%, transparent); }",
    ".dn-history-copy { display: grid; align-content: center; gap: 6px; min-width: 0; }",
    ".dn-history-title strong, .dn-card-title strong { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }",
    ".dn-history-title em { flex: 0 0 auto; color: var(--dn-label); font-size: 0.72rem; font-style: normal; font-weight: 850; text-transform: uppercase; }",
    ".dn-history-detail, .dn-card-meta { display: block; min-width: 0; overflow: hidden; color: var(--dn-muted); font-size: 0.86rem; text-overflow: ellipsis; white-space: nowrap; }",
    ".dn-more { margin-top: 12px; color: var(--dn-label); font-size: 0.82rem; font-weight: 800; }",
    ...
    ".dn-inspector > p { display: -webkit-box; -webkit-line-clamp: 6; -webkit-box-orient: vertical; overflow: hidden; overflow-wrap: anywhere; }",
    ...
    "@media (max-width: 680px) { .dn-shell { padding: 12px; } .dn-header { grid-template-columns: 1fr; padding: 20px; } .dn-header-actions { justify-items: stretch; } .dn-meta { min-width: 0; } .dn-theme-toggle button { min-width: 0; flex: 1; } .dn-signals { grid-template-columns: 1fr; } .dn-panel-heading { align-items: flex-start; flex-direction: column; } .dn-detail-grid { grid-template-columns: 1fr; } }",
    "`;",
    "",
    ...
    "      <div class=\"dn-panel\"><h2>Blockers</h2>${renderBlockers(snapshot.blockers, activeSelection)}</div>",
    "    </section>",
    "  </div>`;",
    "}",
    "",
    ...
    "  return `<button class=\"dn-signal tonebox-${escapeAttribute(signal.tone)} ${id === selectedId ? 'selected' : ''}\" type=\"button\" data-select-id=\"${escapeHtml(id)}\"><span class=\"dn-signal-top\"><span class=\"dn-signal-icon tone-${escapeAttribute(signal.tone)}\">${signalIcon(signal.id)}</span><span class=\"dn-label tone-${escapeAttribute(signal.tone)}\">${escapeHtml(signal.label)}</span></span><strong>${escapeHtml(signal.value)}</strong><p title=\"${escapeHtml(signal.detail)}\">${escapeHtml(signal.detail)}</p></button>`;",
    "}",
    "",
    "function renderEvent(event, selectedId) {",
    "  const relatedId = event.relatedNodeIds.find(Boolean) ?? `event:${event.id}`;",
    "  return `<button class=\"dn-event ${relatedId === selectedId ? 'selected' : ''}\" type=\"button\" data-select-id=\"${escapeHtml(relatedId)}\"><span class=\"dn-label\">${escapeHtml(formatTime(event.time))} · ${escapeHtml(event.source)}</span><strong>${escapeHtml(event.title)}</strong><p>${escapeHtml(event.body)}</p></button>`;",
    "}",
    "",
    "function renderWorkHistory(snapshot, selectedId) {",
    "  const rows = historyRows(snapshot);",
    "  const hiddenCount = Math.max(0, snapshot.weave.nodes.length - rows.length);",
    "  return `<div class=\"dn-panel dn-history-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Branch weave</span><h2>Work History</h2></div><span class=\"dn-count\">${snapshot.weave.nodes.length} nodes · ${snapshot.weave.edges.length} links</span></div><div class=\"dn-branch-board\" role=\"list\">${renderBranchGraph(rows)}<div class=\"dn-history-rows\">${rows.map((row) => renderHistoryItem(row, selectedId)).join('')}${hiddenCount > 0 ? `<div class=\"dn-more\">${hiddenCount} more node(s) folded into details</div>` : ''}</div></div></div>`;",
    "}",
    "",
    ...
    "  return `<button class=\"dn-history-item ${selected} kind-${escapeAttribute(node.kind)}\" style=\"--dn-lane:${row.lane}; --dn-branch-color:var(--dn-branch-${row.lane});\" type=\"button\" data-select-id=\"${escapeHtml(node.id)}\"><span class=\"dn-branch-dot\" aria-hidden=\"true\"></span><span class=\"dn-history-copy\"><span class=\"dn-history-title\"><strong>${escapeHtml(node.label)}</strong><em>${escapeHtml(node.kind)}</em></span><span class=\"dn-history-detail\">${escapeHtml(node.detail || node.status)}</span></span><span class=\"dn-label tone-${escapeAttribute(tone)}\">${escapeHtml(node.status)}</span></button>`;",
    "}",
    "",
    "function renderInspector(snapshot, selectedId) {",
    "  const detail = selectedDetail(snapshot, selectedId);",
    "  return `<aside class=\"dn-panel dn-inspector\"><span class=\"dn-eyebrow\">Details</span><h2>${escapeHtml(truncate(detail.title, 80))}</h2><p title=\"${escapeHtml(detail.body)}\">${escapeHtml(truncate(detail.body, 260))}</p><dl class=\"dn-detail-grid\">${detail.facts.map((fact) => `<div><dt>${escapeHtml(fact[0])}</dt><dd title=\"${escapeHtml(fact[1])}\">${escapeHtml(truncate(fact[1], 90))}</dd></div>`).join('')}</dl>${detail.events.length ? `<div class=\"dn-related\"><span class=\"dn-label\">Related activity</span>${detail.events.slice(0, 3).map((event) => `<article><strong>${escapeHtml(truncate(event.title, 70))}</strong><p>${escapeHtml(truncate(event.body, 140))}</p></article>`).join('')}</div>` : ''}</aside>`;",
    "}",
    "",
    ...
    "}",
    "",
    "function renderBlockers(blockers, selectedId) {",
    "  if (!blockers.length) return '<p>No dashboard-visible blockers.</p>';",
    "  return `<div class=\"dn-blocker-list\">${blockers.slice(0, 8).map((blocker, index) => { const id = `blocker:${index}`; return `<button class=\"dn-blocker ${id === selectedId ? 'selected' : ''}\" type=\"button\" data-select-id=\"${escapeHtml(id)}\"><span class=\"dn-label tone-warn\">Blocker</span><strong>${escapeHtml(blocker)}</strong></button>`; }).join('')}</div>`;",
    "}",
    "",
    ...
    "  return overviewNodes(snapshot).map((node, index) => ({ node, index, lane: laneForNode(node, index) }));",
    "}",
    "",
    "function renderBranchGraph(rows) {",
    "  const rowHeight = 72;",
    ...
    "  const rails = lanes.map((lane) => `<path d=\"M ${xForLane(lane)} 16 V ${height - 16}\" stroke=\"var(--dn-branch-${lane})\" stroke-width=\"3\" opacity=\"0.64\" />`).join('');",
    "  const curves = rows.slice(1).map((row, index) => {",
    "    const previous = rows[index];",
    "    if (!previous || previous.lane === row.lane) return '';",
    "    const y1 = index * rowHeight + rowHeight / 2;",
    "    const y2 = (index + 1) * rowHeight + rowHeight / 2;",
    "    const x1 = xForLane(previous.lane);",
    "    const x2 = xForLane(row.lane);",
    "    const midY = y1 + Math.max(18, (y2 - y1) / 2);",
    "    return `<path d=\"M ${x1} ${y1} C ${x1} ${midY}, ${x2} ${midY}, ${x2} ${y2}\" stroke=\"var(--dn-branch-${row.lane})\" stroke-width=\"3\" opacity=\"0.9\" />`;",
    "  }).join('');",
    "  return `<svg class=\"dn-branch-svg\" width=\"122\" height=\"${height}\" viewBox=\"0 0 122 ${height}\" aria-hidden=\"true\"><path d=\"M 10 16 V ${height - 16}\" stroke=\"var(--dn-branch-faint)\" stroke-width=\"2\" />${rails}${curves}</svg>`;",
    "}",
    "",
    "function laneForNode(node, index) {",
    "  if (node.kind === 'project') return 0;",
    "  if (node.kind === 'branch' || node.kind === 'commit') return 1;",
    "  if (node.kind === 'component' || node.kind === 'tracker') return 2;",
    "  if (node.kind === 'worktree' || node.kind === 'work-item') return 3;",
    "  if (node.kind === 'target-cycle' || node.kind === 'run') return 4;",
    "  if (node.kind === 'authority' || node.kind === 'blocker') return 5;",
    "  return index % 6;",
    "}",
    "",
    "function overviewNodes(snapshot) {",
    "  const nodes = snapshot.weave.nodes;",
    "  const byId = new Map(nodes.map((node) => [node.id, node]));",
    "  const picked = new Map();",
    "  const pick = (node) => { if (node && !picked.has(node.id)) picked.set(node.id, node); };",
    "  pick(byId.get('project'));",
    "  nodes.filter((node) => node.kind !== 'blocker' && ['blocked', 'failed', 'dirty', 'missing'].includes(node.status)).slice(0, 4).forEach(pick);",
    "  snapshot.events.slice(0, 6).forEach((event) => event.relatedNodeIds.forEach((id) => pick(byId.get(id))));",
    "  nodes.filter((node) => node.kind === 'component').slice(0, 5).forEach(pick);",
    "  nodes.filter((node) => node.kind === 'worktree').slice(0, 3).forEach(pick);",
    "  nodes.filter((node) => node.kind === 'target-cycle' || node.kind === 'run').slice(-3).reverse().forEach(pick);",
    "  nodes.filter((node) => node.kind === 'blocker').slice(0, 3).forEach(pick);",
    ...
    "  const facts = [['Kind', node?.kind ?? 'unknown'], ['Status', node?.status ?? 'unknown'], ['Lane', lane?.label ?? node?.laneId ?? 'unknown']];",
    "  if (node?.timestamp) facts.push(['Time', formatTime(node.timestamp)]);",
    "  if (node?.href) facts.push(['Link', node.href]);",
    "  enrichNodeFacts(snapshot, node, facts);",
    "  return { title: node?.label ?? snapshot.project.name, body: node?.detail ?? snapshot.summary, facts, events: relatedEvents(snapshot, node?.id) };",
    "}",
    "",
    "function signalDetail(snapshot, id) {",
    "  const signal = snapshot.signals.find((candidate) => `signal:${candidate.id}` === id) ?? snapshot.signals[0];",
    "  return { title: signal?.label ?? 'Signal', body: signal?.detail ?? snapshot.summary, facts: [['Value', signal?.value ?? 'unknown'], ['Tone', signal?.tone ?? 'neutral'], ['Project', snapshot.project.name]], events: snapshot.events.slice(0, 2) };",
    "}",
    "",
