...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "  return `<article class=\"dn-plugin-card\"><div class=\"dn-plugin-card-header\"><strong>${escapeHtml(plugin.name)}</strong><span class=\"dn-thread-decision decision-${stateClass}\">${escapeHtml(state)}</span></div><span class=\"dn-card-meta\">${escapeHtml(plugin.id)}${escapeHtml(version)}</span><p>${escapeHtml(detail)}</p>${pills}${action}</article>`;",
    "}",
    "",
