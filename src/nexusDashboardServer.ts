...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "    const d = gitHistoryPathD(path, fromX, fromY, toX, toY, y, rowHeight);",
    ...
    "}",
    "",
    "function gitHistoryPathD(path, fromX, fromY, toX, toY, y, rowHeight) {",
    "  if (fromX === toX) return `M ${fromX} ${fromY} V ${toY}`;",
    "  const curve = rowHeight * 0.7;",
    "  const rowGap = Math.abs(path.toIndex - path.fromIndex);",
    "  if (rowGap <= 1) return `M ${fromX} ${fromY} C ${fromX} ${fromY + curve}, ${toX} ${toY - curve}, ${toX} ${toY}`;",
    "  const anchorIndex = path.toIndex > path.fromIndex ? path.toIndex - 1 : path.toIndex + 1;",
    "  const anchorY = y(anchorIndex);",
    "  return `M ${fromX} ${fromY} V ${anchorY} C ${fromX} ${anchorY + curve}, ${toX} ${toY - curve}, ${toX} ${toY}`;",
    "}",
    "",
