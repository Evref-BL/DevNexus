...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ".dn-signal:hover, .dn-component-card:hover, .dn-event:hover, .dn-blocker:hover, .dn-thread-card:hover { transform: translateY(-1px); }",
    ".dn-signal.selected, .dn-component-card.selected, .dn-event.selected, .dn-blocker.selected, .dn-thread-card.selected, .dn-history-item.selected { border-color: var(--dn-active); box-shadow: 0 0 0 2px color-mix(in srgb, var(--dn-active) 18%, transparent) inset; }",
    ...
    ".dn-thread-panel { grid-column: span 2; }",
    ...
    ".dn-rail-labels { position: absolute; left: 0; top: 0; width: 132px; height: 28px; pointer-events: none; }",
    ".dn-rail-labels span { position: absolute; top: 4px; transform: translateX(-50%); color: var(--dn-branch-color); font-size: 0.62rem; font-weight: 900; text-transform: uppercase; white-space: nowrap; }",
    ".dn-branch-svg { position: absolute; left: 0; top: 28px; width: 122px; pointer-events: none; }",
    ".dn-branch-svg path { fill: none; stroke-linecap: round; stroke-linejoin: round; }",
    ".dn-history-rows { display: grid; gap: 0; padding-top: 28px; }",
    ...
    ".dn-component-grid, .dn-blocker-list, .dn-events, .dn-thread-list { display: grid; gap: 10px; max-height: 440px; overflow: auto; }",
    ".dn-event-card, .dn-blocker-card { display: grid; gap: 6px; min-width: 0; }",
    ".dn-thread-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 8px; align-items: center; min-width: 0; }",
    ".dn-component-card, .dn-event, .dn-blocker, .dn-thread-card { display: grid; gap: 6px; min-width: 0; padding: 11px; border: 1px solid var(--dn-border-muted); border-radius: 8px; color: inherit; background: var(--dn-surface-muted); text-align: left; cursor: pointer; transition: transform 160ms ease, border-color 160ms ease, background 160ms ease; }",
    ".dn-thread-card { grid-template-columns: minmax(0, 1fr) auto; align-items: center; }",
    ".dn-thread-main { display: grid; gap: 4px; min-width: 0; }",
    ".dn-thread-main strong { overflow: hidden; color: var(--dn-strong); text-overflow: ellipsis; white-space: nowrap; }",
    ...
    "@media (max-width: 1120px) { .dn-signals { grid-template-columns: repeat(3, minmax(0, 1fr)); } .dn-grid, .dn-main-grid, .dn-secondary-grid { grid-template-columns: 1fr; } .dn-thread-panel { grid-column: auto; } .dn-inspector { position: static; } }",
    "@media (max-width: 680px) { .dn-shell { padding: 12px; } .dn-header { grid-template-columns: 1fr; padding: 20px; } .dn-header-actions { justify-items: stretch; } .dn-meta { min-width: 0; } .dn-theme-toggle button { min-width: 0; flex: 1; } .dn-signals { grid-template-columns: 1fr; } .dn-panel-heading { align-items: flex-start; flex-direction: column; } .dn-history-item, .dn-thread-row { grid-template-columns: minmax(0, 1fr) auto; } .dn-history-detail, .dn-thread-row .dn-action-strip { display: none; } .dn-detail-grid { grid-template-columns: 1fr; } }",
    "`;",
    "",
    ...
    "      ${renderThreadInbox(snapshot, activeSelection)}",
    ...
    "  return `<div class=\"dn-panel dn-history-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Parallel work map</span><h2>Workspace Activity</h2><p class=\"dn-history-note\">Source, worktrees, cycles, and human approvals in one view.</p></div><span class=\"dn-count\">${rows.length} rows · ${timeline.lanes.length} lanes</span></div>${renderLaneKey(timeline.lanes)}<div class=\"dn-branch-board\" role=\"list\">${renderRailLabels(timeline.lanes)}${renderBranchGraph(rows, timeline.lanes)}<div class=\"dn-history-rows\">${rows.map((row) => renderHistoryItem(row, selectedId)).join('')}</div></div></div>`;",
    "}",
    "",
    ...
    "}",
    "",
    "function renderRailLabels(lanes) {",
    "  return `<div class=\"dn-rail-labels\" aria-hidden=\"true\">${lanes.map((lane) => `<span style=\"left:${22 + lane.index * 18}px; --dn-branch-color:var(--dn-branch-${lane.index});\" title=\"${escapeHtml(lane.label)}\">${escapeHtml(lane.railLabel ?? lane.shortLabel)}</span>`).join('')}</div>`;",
    "}",
    "",
    ...
    "  const count = snapshot.threads ? `${snapshot.threads.needsDecisionCount} decisions · ${snapshot.threads.forgetCandidateCount} forget` : '0 decisions';",
    "  const body = threads.length ? threads.slice(0, 8).map((thread) => renderThreadCard(thread, selectedId)).join('') : '<p>No open threads.</p>';",
    "  return `<div class=\"dn-panel dn-thread-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Codex threads</span><h2>Thread Inbox</h2></div><span class=\"dn-count\">${escapeHtml(count)}</span></div><div class=\"dn-thread-list\">${body}</div></div>`;",
    "}",
    "",
    ...
    "  return `<article class=\"dn-thread-row\"><button class=\"dn-thread-card ${selected}\" type=\"button\" data-select-id=\"${escapeHtml(id)}\" title=\"${escapeHtml(thread.decisionDetail)}\"><span class=\"dn-thread-main\"><strong>${escapeHtml(thread.title)}</strong><span class=\"dn-card-meta\">${escapeHtml(meta)}</span></span><span class=\"dn-thread-decision decision-${escapeAttribute(thread.decision)}\">${escapeHtml(thread.decisionLabel)}</span></button>${renderActionStrip(thread.actions, 'compact')}</article>`;",
    "}",
    "",
    ...
    "  const lanes = [{ key: 'main', label: `source: ${snapshot.project.defaultBranch ?? 'main'}`, shortLabel: 'main', railLabel: 'main', index: 0 }];",
    ...
    "    seen.add(key);",
    "    lanes.push({ key, label: `active worktree: ${compactBranchName(worktree.branchName ?? worktree.id)}`, shortLabel: 'worktree', railLabel: `w${lanes.length}`, index: lanes.length });",
    "  }",
    "  lanes.push({ key: 'worktrees', label: 'other worktrees', shortLabel: 'other worktrees', railLabel: 'other', index: lanes.length });",
    "  lanes.push({ key: 'cycles', label: 'target cycles and runs', shortLabel: 'cycles and runs', railLabel: 'cycles', index: lanes.length });",
    "  lanes.push({ key: 'policy', label: 'human approval and blockers', shortLabel: 'human approval', railLabel: 'human', index: lanes.length });",
    ...
    "  if (String(id).startsWith('thread:')) return snapshot.threads?.records?.some((thread) => `thread:${thread.id}` === id) ?? false;",
    ...
    "  if (node.kind === 'authority') return 'Human approval';",
    "  return node.label;",
    "}",
    "",
    "function displayBody(node, snapshot) {",
    "  if (!node) return snapshot.summary;",
    "  if (node.kind === 'authority') return node.detail || 'A provider action needs human review before automation can continue.';",
    "  if (node.kind === 'blocker') return readableBlocker(node.detail);",
    "  if (node.kind === 'worktree') { const thread = threadForNode(snapshot, node); if (thread) return `${thread.decisionLabel}: ${thread.decisionDetail}`; }",
    ...
    "  if (id === 'signal:worktrees' && snapshot.threads) return { title: 'Threads', body: signal?.detail ?? 'Thread status', facts: [['Open', String(snapshot.threads.totalCount)], ['Needs review', String(snapshot.threads.needsDecisionCount)], ['Archive', String(snapshot.threads.archiveCandidateCount)], ['Forget', String(snapshot.threads.forgetCandidateCount)]], events: [], actions: uniqueActions(snapshot.threads.records.flatMap((thread) => thread.actions ?? [])) };",
    ...
    "  const thread = snapshot.threads?.records?.find((candidate) => `thread:${candidate.id}` === id);",
    "  if (!thread) return { title: 'Thread', body: 'Thread not found.', facts: [], events: [], actions: [] };",
    "  const facts = [['Decision', thread.decisionLabel], ['State', thread.state || 'unknown'], ['Component', thread.componentId ?? 'workspace'], ['Work item', thread.workItemId ?? 'none'], ['Branch', thread.branchName ?? 'none'], ['Cleanup proof', thread.cleanupSafe === true ? 'safe to forget' : thread.cleanupSafe === false ? 'blocked' : 'not checked']];",
    "  if (thread.hostId) facts.push(['Host', thread.hostId]);",
    "  if (thread.updatedAt) facts.push(['Updated', formatTime(thread.updatedAt)]);",
    "  return { title: thread.title, body: thread.decisionDetail, facts, events: [], actions: thread.actions ?? [] };",
    "}",
    "",
    ...
    "    const thread = threadForNode(snapshot, node);",
    "    if (worktree) {",
    "      if (thread) facts.push(['Decision', thread.decisionLabel]);",
    ...
    "      if (thread) facts.push(['Cleanup proof', thread.cleanupSafe === true ? 'safe to forget' : thread.cleanupSafe === false ? 'blocked' : 'not checked']);",
    ...
    "    facts.push(['Human steps', String(snapshot.authority.fallbackActionCount)]);",
    "  }",
    "}",
    "",
    "function threadForNode(snapshot, node) {",
    "  return snapshot.threads?.records?.find((thread) => `worktree:${thread.id}` === node?.id || `thread:${thread.id}` === node?.id) ?? null;",
    "}",
    "",
    ...
    "  if (/No resolved auth profile is available for publication action provider\\.pull_request\\.open/iu.test(text)) return 'No bot credential is available for opening a pull request. Human approval is required.';",
    "  return text.replace(/provider\\.pull_request\\.open/gu, 'opening a pull request').replace(/coordination\\.handoff/gu, 'human approval');",
    "}",
    "",
