...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ":root { --dn-grid-line: rgba(180, 210, 188, 0.055); --dn-shadow: 0 18px 50px rgba(0, 0, 0, 0.22); }",
    ":root[data-dev-nexus-theme='light'] { --dn-grid-line: rgba(31, 115, 93, 0.085); --dn-shadow: 0 18px 40px rgba(34, 50, 42, 0.09); }",
    ...
    "body { margin: 0; min-width: 320px; color: var(--dn-text); background: linear-gradient(90deg, var(--dn-grid-line) 1px, transparent 1px), linear-gradient(0deg, var(--dn-grid-line) 1px, transparent 1px), var(--dn-bg); background-size: 28px 28px; }",
    "button, input, select { font: inherit; }",
    ".dn-shell { width: min(1520px, 100%); margin: 0 auto; padding: 24px; }",
    ".dn-header { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 24px; align-items: end; min-height: 190px; padding: 28px; border: 1px solid var(--dn-border); border-radius: 8px; background: color-mix(in srgb, var(--dn-surface) 92%, transparent); box-shadow: var(--dn-shadow); }",
    ...
    ".dn-signal { min-height: 132px; padding: 14px; text-align: left; color: inherit; cursor: pointer; }",
    ...
    ".dn-history-panel { min-height: 560px; }",
    ".dn-count { color: var(--dn-label); font-size: 0.8rem; font-weight: 800; white-space: nowrap; }",
    ".dn-history-list { display: grid; gap: 0; margin-top: 12px; }",
    ".dn-history-item { display: grid; grid-template-columns: 58px minmax(0, 1fr); gap: 12px; min-height: 72px; padding: 0; border: 0; border-radius: 8px; color: inherit; background: transparent; text-align: left; cursor: pointer; }",
    ".dn-history-item:hover { background: var(--dn-control-hover); }",
    ".dn-graph-rail { position: relative; display: grid; place-items: center; min-height: 72px; }",
    ".dn-graph-rail i { position: absolute; inset-block: 0; width: 2px; background: var(--dn-border-strong); }",
    ".dn-graph-rail b { position: relative; display: grid; place-items: center; width: 30px; height: 30px; border: 2px solid currentColor; border-radius: 999px; background: var(--dn-surface); color: currentColor; font-size: 0.72rem; font-style: normal; }",
    ".dn-history-copy { display: grid; align-content: center; gap: 6px; min-width: 0; border-bottom: 1px solid var(--dn-border-muted); }",
    ...
    ".dn-more { margin: 10px 0 0 58px; color: var(--dn-label); font-size: 0.82rem; font-weight: 800; }",
    ".dn-inspector { position: sticky; top: 16px; align-self: start; }",
    ...
    ".dn-component-card, .dn-event, .dn-blocker { display: grid; gap: 6px; min-width: 0; padding: 11px; border: 1px solid var(--dn-border-muted); border-radius: 8px; color: inherit; background: var(--dn-surface-muted); text-align: left; cursor: pointer; }",
    ...
    "  return `<button class=\"dn-signal ${id === selectedId ? 'selected' : ''}\" type=\"button\" data-select-id=\"${escapeHtml(id)}\"><span class=\"dn-signal-top\"><span class=\"dn-dot tone-${escapeAttribute(signal.tone)}\"></span><span class=\"dn-label tone-${escapeAttribute(signal.tone)}\">${escapeHtml(signal.label)}</span></span><strong>${escapeHtml(signal.value)}</strong><p title=\"${escapeHtml(signal.detail)}\">${escapeHtml(signal.detail)}</p></button>`;",
    "}",
    "",
    ...
    "  const items = overviewNodes(snapshot);",
    "  const hiddenCount = Math.max(0, snapshot.weave.nodes.length - items.length);",
    "  return `<div class=\"dn-panel dn-history-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Weave</span><h2>Work History</h2></div><span class=\"dn-count\">${snapshot.weave.nodes.length} nodes · ${snapshot.weave.edges.length} links</span></div><div class=\"dn-history-list\" role=\"list\">${items.map((node, index) => renderHistoryItem(node, index, selectedId)).join('')}${hiddenCount > 0 ? `<div class=\"dn-more\">${hiddenCount} more node(s) folded into details</div>` : ''}</div></div>`;",
    "}",
    "",
    "function renderHistoryItem(node, index, selectedId) {",
    "  const tone = toneForStatus(node.status, node.kind);",
    "  const selected = node.id === selectedId ? 'selected' : '';",
    "  return `<button class=\"dn-history-item ${selected} kind-${escapeAttribute(node.kind)}\" type=\"button\" data-select-id=\"${escapeHtml(node.id)}\"><span class=\"dn-graph-rail\" aria-hidden=\"true\"><i></i><b class=\"tone-${escapeAttribute(tone)}\">${index + 1}</b></span><span class=\"dn-history-copy\"><span class=\"dn-history-title\"><strong>${escapeHtml(node.label)}</strong><em>${escapeHtml(node.kind)}</em></span><span class=\"dn-history-detail\">${escapeHtml(node.detail || node.status)}</span></span></button>`;",
    "}",
    "",
