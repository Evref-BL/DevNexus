...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ".dn-header { position: relative; display: grid; grid-template-columns: minmax(260px, 1fr) minmax(0, 760px); gap: 24px; align-items: end; min-height: 190px; overflow: hidden; padding: 32px; border: 1px solid var(--dn-border); border-radius: 8px; background: linear-gradient(135deg, color-mix(in srgb, var(--dn-surface) 88%, var(--dn-branch-0) 12%), color-mix(in srgb, var(--dn-surface) 92%, var(--dn-branch-5) 8%) 52%, color-mix(in srgb, var(--dn-surface) 90%, var(--dn-branch-1) 10%)); box-shadow: var(--dn-shadow); }",
    ...
    ".dn-host-header-actions { width: min(100%, 760px); }",
    ...
    ".dn-header-pill { display: grid; gap: 3px; min-height: 46px; padding: 8px 10px; border: 1px solid color-mix(in srgb, var(--dn-active) 22%, var(--dn-border)); border-radius: 8px; background: color-mix(in srgb, var(--dn-surface) 72%, var(--dn-active) 12%); }",
    ...
    ".dn-header-path-row { display: flex; align-items: stretch; gap: 8px; min-width: min(100%, 430px); flex: 1 1 430px; }",
    ".dn-header-path { flex: 1 1 auto; min-width: 0; }",
    ...
    ".dn-open-trigger { --dn-action-accent: var(--dn-active); justify-content: space-between; min-width: 94px; min-height: 38px; gap: 7px; padding: 5px 5px 5px 10px; border-color: color-mix(in srgb, var(--dn-border-strong) 78%, var(--dn-active) 22%); background: color-mix(in srgb, var(--dn-surface) 92%, var(--dn-active) 8%); box-shadow: none; }",
    ".dn-open-trigger:hover { transform: none; border-color: color-mix(in srgb, var(--dn-border-strong) 46%, var(--dn-active) 54%); background: color-mix(in srgb, var(--dn-surface) 86%, var(--dn-active) 14%); }",
    ".dn-open-menu[open] .dn-open-trigger { border-color: color-mix(in srgb, var(--dn-border-strong) 34%, var(--dn-active) 66%); background: color-mix(in srgb, var(--dn-surface) 82%, var(--dn-active) 18%); }",
    ...
    ".dn-open-option svg, .dn-open-trigger svg { flex: 0 0 auto; width: 16px; height: 16px; }",
    ".dn-open-option svg:not(.dn-app-icon), .dn-open-trigger svg { stroke: currentColor; fill: none; stroke-width: 1.9; stroke-linecap: round; stroke-linejoin: round; }",
    ...
    "@media (max-width: 860px) { .dn-header { grid-template-columns: 1fr; } .dn-header-actions { justify-content: stretch; width: 100%; } .dn-header-strip, .dn-header-path-row { width: 100%; } }",
    ...
    "  const workHistory = loading && !threadsLoaded ? renderProgressivePanel('parallel-work-map', 'Parallel work map', 'Workspace Activity', 'Loading checkout lanes, worktrees, cycles, and approvals.') : renderWorkHistory(snapshot, activeSelection);",
    ...
    "  return `<div class=\"dn-shell dn-host-dashboard\"><header class=\"dn-header\"><div><span class=\"dn-eyebrow\">DevNexus cockpit</span><h1>Host Cockpit</h1><p>${escapeHtml(summary)}</p></div>${renderHostHeaderActions(host, themeMode)}</header>${renderHostSignals(host, focus)}<section class=\"dn-host-main-grid\">${renderHostActionQueue(host, focus)}${renderHostOverview(host, null, '', { hostMode: true, focus })}</section></div>`;",
    "}",
    "",
    "function renderHostHeaderActions(host, themeMode) {",
    "  const homePath = host?.homePath ?? '';",
    "  return `<div class=\"dn-header-actions dn-host-header-actions\"><div class=\"dn-header-strip\"><span class=\"dn-header-pill dn-host-identity\"><span>Host</span><strong title=\"${escapeHtml(hostIdentity(host))}\">${escapeHtml(hostIdentity(host))}</strong></span><span class=\"dn-header-pill dn-header-stamp\"><span>Generated</span><strong>${escapeHtml(formatTime(host?.generatedAt))}</strong></span><div class=\"dn-header-path-row\"><span class=\"dn-header-pill dn-header-path\"><span>Home</span><strong title=\"${escapeHtml(homePath)}\">${escapeHtml(compactPath(homePath))}</strong></span>${renderOpenMenu('home', 'Open')}</div></div>${renderThemeToggle(themeMode)}</div>`;",
    "}",
    "",
    ...
    "  return `<div class=\"dn-panel dn-history-panel\" id=\"parallel-work-map\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Parallel work map</span><h2>Workspace Activity</h2><p class=\"dn-history-note\">Checkout lanes, worktrees, cycles, and approvals in one view.</p></div><span class=\"dn-count\">${countLabel(rows.length, 'row')} · ${countLabel(timeline.lanes.length, 'lane')}</span></div>${renderLaneKey(timeline.lanes)}<div class=\"dn-branch-board\" role=\"list\">${renderBranchGraph(rows, timeline.lanes)}<div class=\"dn-history-rows\">${rows.map((row) => renderHistoryItem(row, selectedId)).join('')}</div></div></div>`;",
    "}",
    "",
    ...
    "  lanes.push({ key: 'worktrees', label: 'Other worktrees', shortLabel: 'Worktrees', detail: remainingWorktrees ? countLabel(remainingWorktrees, 'more branch', 'more branches') : 'Grouped branches', index: lanes.length });",
