...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ".dn-git-graph { display: block; width: 118px; min-height: 34px; border-right: 1px solid var(--dn-border-muted); }",
    ...
    "  const gitHistory = loading && !componentsLoaded ? renderProgressivePanel('project-git-history', 'Git history', 'Project History', 'Loading commits, refs, and branch heads.') : renderGitHistory(snapshot, activeSelection);",
    ...
    "}",
    "",
    "function renderGitHistory(snapshot, selectedId) {",
    "  const graph = gitHistoryRows(snapshot);",
    ...
    "  const count = `${countLabel(repository.commits.length, 'commit')} · ${countLabel(repository.branchNames?.length ?? 0, 'branch')}`;",
    "  const note = repository.moreAvailable ? `<p class=\"dn-git-note\">Showing the newest ${repository.commits.length} commits.</p>` : '';",
    "  return `<div class=\"dn-panel dn-git-panel\" id=\"project-git-history\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Git history</span><h2>Project History</h2><p class=\"dn-history-note\">Git refs and commit parent relationships are the source of truth.</p></div><span class=\"dn-count\">${escapeHtml(count)}</span></div><div class=\"dn-git-board\">${renderGitHistorySvg(graph)}<div class=\"dn-git-rows\">${graph.rows.map((row) => renderGitHistoryRow(row, selectedId)).join('')}</div></div>${note}</div>`;",
    "}",
    "",
    "function gitHistoryRows(snapshot) {",
    "  const repository = (snapshot.history?.repositories ?? []).find((candidate) => (candidate.commits ?? []).length > 0) ?? null;",
    "  if (!repository) return null;",
    "  const commits = (repository.commits ?? []).slice(0, 80);",
    ...
    "    }",
    "    rows.push({ repository, commit, lane, index: rows.length, selectId: gitHistorySelectId(repository, commit) });",
    ...
    "      active[firstOpenLane(active)] = parent;",
    "    }",
    "    trimEmptyLanes(active);",
    "  }",
    ...
    "      if (target) paths.push({ fromLane: row.lane, toLane: target.lane, fromIndex: row.index, toIndex: target.index });",
    "    }",
    "  }",
    "  const maxLane = Math.max(0, ...rows.map((row) => row.lane), ...paths.flatMap((path) => [path.fromLane, path.toLane]));",
    "  return { repository, rows, paths, maxLane };",
    "}",
    "",
    "function firstOpenLane(active) {",
    "  const index = active.findIndex((value) => value === null);",
    "  return index >= 0 ? index : active.length;",
    "}",
    "",
    "function trimEmptyLanes(active) {",
    ...
    "  const width = Math.max(84, offsetX * 2 + graph.maxLane * laneGap);",
    ...
    "    const fromX = x(path.fromLane);",
    "    const fromY = y(path.fromIndex);",
    "    const toX = x(path.toLane);",
    "    const toY = y(path.toIndex);",
    "    const color = `var(--dn-branch-${path.fromLane % 7})`;",
    "    const d = fromX === toX ? `M ${fromX} ${fromY} V ${toY}` : `M ${fromX} ${fromY} C ${fromX} ${fromY + rowHeight * 0.7}, ${toX} ${toY - rowHeight * 0.7}, ${toX} ${toY}`;",
    "    return `<path d=\"${d}\" stroke=\"${color}\" stroke-width=\"3\" opacity=\"0.78\" />`;",
    "  }).join('');",
    "  const dots = graph.rows.map((row) => `<circle cx=\"${x(row.lane)}\" cy=\"${y(row.index)}\" r=\"4.5\" fill=\"var(--dn-surface)\" stroke=\"var(--dn-branch-${row.lane % 7})\" stroke-width=\"2.5\" />`).join('');",
    "  return `<svg class=\"dn-git-graph\" viewBox=\"0 0 ${width} ${height}\" role=\"img\" aria-label=\"Git commit graph\">${paths}${dots}</svg>`;",
    "}",
    "",
    "function renderGitHistoryRow(row, selectedId) {",
    "  const selected = row.selectId === selectedId ? ' selected' : '';",
    "  const refs = (row.commit.refs ?? []).filter((ref) => ref.kind !== 'head').slice(0, 3);",
    "  const refChips = refs.map((ref) => `<span class=\"dn-git-ref\" style=\"--dn-branch-color:var(--dn-branch-${row.lane % 7});\" title=\"${escapeHtml(ref.name)}\">${escapeHtml(ref.name)}</span>`).join('');",
    "  const meta = [row.commit.authorName, formatTime(row.commit.committedAt)].filter(Boolean).join(' · ');",
    "  return `<button class=\"dn-git-history-row${selected}\" type=\"button\" data-select-id=\"${escapeHtml(row.selectId)}\" data-scroll-target=\"selected-item\"><span class=\"dn-git-subject\"><span class=\"dn-git-refs\">${refChips}</span><strong title=\"${escapeHtml(row.commit.subject)}\">${escapeHtml(row.commit.subject)}</strong></span><span class=\"dn-git-meta\">${escapeHtml(meta)}</span><span class=\"dn-git-sha\">${escapeHtml(row.commit.shortHash)}</span></button>`;",
    "}",
    "",
