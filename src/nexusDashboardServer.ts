...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "      <div class=\"dn-work-stack\">${renderWorkHistory(snapshot, activeSelection)}${renderThreadInbox(snapshot)}${renderTrackedWork(snapshot)}</div>",
    "    </section>",
    ...
    "      }",
    "      setTimeout(() => { delete button.dataset.copied; if (label) label.textContent = 'Copy prompt'; }, 1600);",
    "    });",
    "  });",
    ...
    "  const count = snapshot.threads ? `${snapshot.threads.needsDecisionCount} need action` : '0 need action';",
    ...
    "}",
    "",
    "function renderTrackedWork(snapshot) {",
    ...
    "  const body = records.length ? records.slice(0, 8).map(renderTrackedWorkCard).join('') : '<p>No tracked work is waiting.</p>';",
    ...
    "}",
    "",
    "function renderTrackedWorkCard(item) {",
    ...
    "  return `<article class=\"dn-tracked-card kind-${escapeAttribute(item.kind)}\"><div class=\"dn-tracked-card-header\"><span class=\"dn-thread-main\"><strong title=\"${escapeHtml(item.title)}\">${escapeHtml(item.title)}</strong><span class=\"dn-card-meta\">${escapeHtml(meta)}</span></span><span class=\"dn-thread-decision decision-${trackedWorkDecisionClass(item)}\">${escapeHtml(item.kindLabel)}</span></div><p title=\"${escapeHtml(detail)}\">${escapeHtml(detail)}</p>${renderActionStrip(item.actions, 'compact')}</article>`;",
    "}",
    "",
    ...
    "  const disabled = records.filter((plugin) => !plugin.enabled).length;",
    "  const count = plugins ? `${plugins.enabledCount} enabled${disabled ? ` · ${disabled} disabled` : ''} · ${plugins.capabilityCount} capabilities` : '0 enabled';",
    "  const body = records.length ? records.map(renderPluginCard).join('') : '<p>No DevNexus plugins installed.</p>';",
    "  return `<div class=\"dn-panel dn-plugin-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Installed extensions</span><h2>Plugins</h2></div><span class=\"dn-count\">${escapeHtml(count)}</span></div><div class=\"dn-plugin-list\">${body}</div><p class=\"dn-plugin-note\">Install actions appear only after a trusted plugin source and policy are configured.</p></div>`;",
    "}",
    "",
    "function renderPluginCard(plugin) {",
    "  const version = plugin.version ? ` · ${plugin.version}` : '';",
    "  const state = plugin.enabled ? 'enabled' : 'disabled';",
    "  const stateClass = plugin.enabled ? 'continue' : 'archive';",
    ...
    "  return `<article class=\"dn-plugin-card\"><div class=\"dn-plugin-card-header\"><strong>${escapeHtml(plugin.name)}</strong><span class=\"dn-thread-decision decision-${stateClass}\">${escapeHtml(state)}</span></div><span class=\"dn-card-meta\">${escapeHtml(plugin.id)}${escapeHtml(version)}</span><p>${escapeHtml(detail)}</p>${pills}</article>`;",
    "}",
    "",
