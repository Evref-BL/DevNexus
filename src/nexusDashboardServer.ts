...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "    }",
    "    if (latestSnapshot) {",
    "      renderRoot(renderDashboard(latestSnapshot, themeMode, selectedId, latestHost, selectedWorkspaceId, gitHistoryFilter));",
    "    } else if (latestError) {",
    ...
    "  const count = `${countLabel(graph.rows.length, 'commit')} · ${countLabel(repository.branchNames?.length ?? 0, 'branch')}`;",
    ...
    "  const commits = filteredGitHistoryCommits(snapshot, repository, normalizeGitHistoryFilter(filter)).slice(0, 80);",
