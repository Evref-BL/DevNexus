...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "  const title = `${node.label} · ${node.detail || node.status}`;",
    "  return `<button class=\"dn-history-item ${selected} kind-${escapeAttribute(node.kind)}\" style=\"--dn-lane:${row.lane}; --dn-branch-color:var(--dn-branch-${row.lane});\" type=\"button\" data-lane=\"${row.lane}\" data-select-id=\"${escapeHtml(node.id)}\" title=\"${escapeHtml(title)}\"><span class=\"dn-branch-dot\" aria-hidden=\"true\"></span><span class=\"dn-history-main\"><span class=\"dn-history-chip\">${escapeHtml(node.kind)}</span><strong>${escapeHtml(node.label)}</strong></span><span class=\"dn-history-detail\">${escapeHtml(node.detail || node.status)}</span><span class=\"dn-history-status tone-${escapeAttribute(tone)}\">${escapeHtml(node.status)}</span></button>`;",
    "}",
    "",
    ...
    "  const rails = lanes.map((lane) => `<path d=\"M ${xForLane(lane)} ${railTop} V ${railBottom}\" stroke=\"var(--dn-branch-${lane})\" stroke-width=\"3\" opacity=\"0.64\" />`).join('');",
    ...
    "  }).join('');",
    "  return `<svg class=\"dn-branch-svg\" width=\"122\" height=\"${height}\" viewBox=\"0 0 122 ${height}\" aria-hidden=\"true\" data-row-height=\"${rowHeight}\"><path d=\"M 10 ${railTop} V ${railBottom}\" stroke=\"var(--dn-branch-faint)\" stroke-width=\"2\" />${rails}${curves}</svg>`;",
    "}",
    "",
    ...
    "  nodes.filter((node) => node.kind !== 'blocker' && ['blocked', 'failed', 'dirty', 'missing'].includes(node.status)).slice(0, 6).forEach(pick);",
    "  snapshot.events.slice(0, 10).forEach((event) => event.relatedNodeIds.forEach((id) => pick(byId.get(id))));",
    "  nodes.filter((node) => node.kind === 'component').slice(0, 5).forEach(pick);",
    "  nodes.filter((node) => node.kind === 'worktree').slice(0, 8).forEach(pick);",
    "  nodes.filter((node) => node.kind === 'target-cycle' || node.kind === 'run').slice(-12).reverse().forEach(pick);",
    "  nodes.filter((node) => node.kind === 'blocker').slice(0, 8).forEach(pick);",
    "  nodes.forEach((node) => { if (picked.size < 36) pick(node); });",
