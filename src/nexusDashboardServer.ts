...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "  return `<div class=\"dn-panel dn-history-panel\" id=\"parallel-work-map\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Parallel work map</span><h2>Workspace Activity</h2><p class=\"dn-history-note\">Checkout lanes, worktrees, cycles, and approvals in one view.</p></div><span class=\"dn-count\">${rows.length} rows · ${timeline.lanes.length} lanes</span></div>${renderLaneKey(timeline.lanes)}<div class=\"dn-branch-board\" role=\"list\">${renderBranchGraph(rows, timeline.lanes)}<div class=\"dn-history-rows\">${rows.map((row) => renderHistoryItem(row, selectedId)).join('')}</div></div></div>`;",
    "}",
    "",
