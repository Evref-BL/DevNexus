...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    "      <div class=\"dn-work-stack\">${features}${workHistory}${threadInbox}${trackedWork}</div>",
    "    </section>",
    ...
    "  const feature = (snapshot.features?.records ?? []).find((candidate) => candidate.status === 'blocked' || candidate.status === 'needs-review') ?? (snapshot.features?.records ?? [])[0];",
