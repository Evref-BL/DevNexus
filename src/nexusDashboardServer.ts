...
export function renderNexusDashboardClientModule(): string {
  return [
    ...
    ".dn-side-stack { display: grid; gap: 14px; align-content: start; min-width: 0; }",
    ...
    ".dn-lane-key span { max-width: 132px; padding: 3px 7px; overflow: hidden; border: 1px solid var(--dn-border-muted); border-left: 5px solid var(--dn-branch-color); border-radius: 6px; color: var(--dn-muted); background: var(--dn-surface-muted); font-size: 0.72rem; font-weight: 800; text-overflow: ellipsis; white-space: nowrap; }",
    ...
    "      <div class=\"dn-side-stack\">${renderThreadInbox(snapshot)}${renderInspector(snapshot, activeSelection)}${renderPlugins(snapshot.plugins)}</div>",
    "    </section>",
    ...
    "  return `<div class=\"dn-panel dn-plugin-panel\"><div class=\"dn-panel-heading\"><div><span class=\"dn-eyebrow\">Capability layer</span><h2>Plugins</h2></div><span class=\"dn-count\">${escapeHtml(count)}</span></div><div class=\"dn-plugin-list\">${body}</div></div>`;",
    "}",
    "",
    ...
    "  const lanes = [{ key: 'main', label: `source: ${snapshot.project.defaultBranch ?? 'main'}`, shortLabel: 'main', index: 0 }];",
    ...
    "    seen.add(key);",
    "    lanes.push({ key, label: `active branch: ${compactBranchName(worktree.branchName ?? worktree.id)}`, shortLabel: 'active', index: lanes.length });",
    "  }",
    "  lanes.push({ key: 'worktrees', label: 'other branches', shortLabel: 'other', index: lanes.length });",
