...
async function handleDashboardCommand(
  ...
  if (command === "serve") {
    ...
    if (parsed.restart) {
      if (!currentProjectRoot) {
        throw new Error(
          "dashboard serve --restart requires a workspace root or current workspace",
        );
      }
      if (!parsed.port || parsed.port <= 0) {
        throw new Error("dashboard serve --restart requires a non-zero --port");
      }
      ...
      if (currentProcessSkip) {
        throw new Error(
          `Refusing to restart dashboard server ${currentProcessSkip.record.id} because it belongs to the current process (${currentProcessSkip.record.pid})`,
        );
      }
    }
  ...
  if (command === "status") {
    const parsed = parseDashboardCommand(argv, "dashboard status", {
      requireProjectRoot: false,
    });
    const projectRoot =
      parsed.projectRoot ?? discoverDashboardCurrentProjectRoot(process.cwd());
  ...
  throw new Error("dashboard requires snapshot, weave, serve, or status");
}
...
function printDashboardStatusResult(
  ...
  writeLine(stdout, "DevNexus dashboard server status.");
