...
describe("nexus dashboard", () => {
  ...
  it("explains how to recover when a known dashboard port is occupied", async () => {
    ...
    try {
      ...
      expect(message).toContain(
        `Dashboard port 127.0.0.1:${server.port} is already in use.`,
      );
      expect(message).toContain(server.url);
      expect(message).toContain("dev-nexus dashboard status");
