...
describe("nexus dashboard", () => {
  ...
  it("renders a client module with explicit light and dark mode controls", () => {
    ...
    expect(module).toContain("latestHost.partial !== true");
    expect(module).toContain("Date.now() - lastHostRefreshAt < hostRefreshMs");
    expect(module).toContain("if (!force && hostIsFreshEnough) return;");
  ...
  it("shares provider freshness across dashboard server routes", async () => {
    ...
    try {
      ...
      const dashboard = await fetch(
        `${server.url}api/dashboard?workspace=dashboard-demo`,
      ).then((response) => response.json());

      expect(host.actionQueue).toEqual(expect.any(Array));
      expect(dashboard.project.id).toBe("dashboard-demo");
