...
import {
  ...
  stopProcessByPid,
  waitForHttpPort,
} from "../../src/runtime/processSupervisor.js";
...
describe("process supervisor", () => {
  ...
  it("resolves Windows command shim candidates behind one helper", () => {
    const shimDirectory = makeTempDir("dev-nexus-shim-");
  ...
  it("starts a process, captures pid, and writes stdout/stderr/lifecycle logs", async () => {
    const logDirectory = makeTempDir("dev-nexus-logs-");
    ...
    expect(handle.logPaths.stdout).toBe(
      path.join(logDirectory, "example-service.stdout.log"),
    );
  ...
  it("fails with actionable feedback when the executable is missing", () => {
    ...
    expect(() =>
      startManagedProcess({
        name: "example-service",
        command: "definitely-not-a-real-dev-nexus-command",
        logDirectory,
      }),
    ...
    expect(() =>
      ...
      "Cannot start example-service: executable not found: definitely-not-a-real-dev-nexus-command.",
    );
  });
