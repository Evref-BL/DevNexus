...
- which write events happened
- which branches or tracks existed in parallel
- where tracks split, continued, or merged
- which human decisions explain or authorize a write event
...
Use `write event` for the primary row event currently represented by a Git
commit. Avoid `git event` in the durable model because future sources can
describe writes that are not Git commits.

| Term | Meaning |
| --- | --- |
| Write event | A durable repository or workspace write, such as a commit. |
| Decision event | A human or policy decision, such as approval, rejection, rescue, or block. |
| Track | A time-ordered path of related write events. Git branches are one source of tracks. |
...
| --- | --- | --- |
| Write | commit, generated source write, merge result | Primary row with node, subject, author, time, and hash or id. |
| Decision | approved, rejected, blocked, rescued | Marker or detail attached to the related write event. |
| Review | requested review, review completed, changes requested | Marker or detail attached to the related write event or track. |
...
- every visible write event has one row
...
- connector endpoints touch a write node or routed track segment
...
- one compact row per write event
- branch or track geometry stays visible
- important markers appear inline
- no raw JSON-shaped details

The detailed view opens from a selected write event:

- parent and child write events
...
2. Rename the primary model from commit/git event language to write event
...
- Whether a selected write event opens an inline detail row, a side detail panel,
  or both.
