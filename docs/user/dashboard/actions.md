...
Curated plugin catalogue entries can show `Copy command`. That copies the
explicit `dev-nexus workspace plugin refresh` command for the current
workspace; it does not run install or setup from the browser.
