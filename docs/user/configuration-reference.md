...
`credentialKind`, `cycleLedgerPath`, `defaultBranch`, `defaultBranchStrategy`, `defaultCorePack`, `defaultIntentPrefix`, `defaultToolsApprovalMode`, `defaultWorkTrackerId`
`dependencyLinks`, `description`, `directExternalSelection`, `directTargetPush`, `directory`, `displayName`, `diverged`, `driver`
`eligibleWorkMode`, `email`, `enabled`, `endpoint`, `environment`, `environmentKeys`, `ephemeralThreadDefault`, `excludeLabels`
`excludedTools`, `executor`, `executorMode`, `exposure`, `extensions`, `failureLimit`, `fallbackRemote`, `featureBranchDelivery`
`featureBranchPattern`, `finalLimit`, `finalPullRequest`, `finalPullRequestCreation`, `fingerprints`, `flow`, `focusedCommands`, `fullCommands`
`gates`, `gateway`, `gitIdentity`, `gitUserEmail`, `gitUserName`, `gitWorkflows`, `githubApp`, `githubCliConfigDir`
`greenMain`, `handle`, `handles`, `heartbeatIntervalMs`, `home`, `host`, `hostId`, `hostLocalSafetyHints`
`hostOverlays`, `hosting`, `hosts`, `id`, `identityRef`, `importRequiredFirst`, `includedServers`, `includedTools`
`installCommand`, `installationAccount`, `integrationBranch`, `integrationPreference`, `integrationPrefix`, `intelligence`, `intendedUse`, `intervalMs`
`invitationPolicy`, `issueType`, `itemId`, `itemKey`, `itemNumber`, `items`, `kind`, `labels`
`leaseDurationMs`, `ledger`, `license`, `limit`, `limits`, `localPolicy`, `lock`, `manualActor`
`manualInstructions`, `manualRemote`, `materialization`, `maxConcurrentSubagents`, `maxCycles`, `maxDelayMs`, `maxWorkItems`, `mcp`
`mcpExposure`, `mergeAuthority`, `milestones`, `missingCredentialBehavior`, `mode`, `model`, `mutationClass`, `name`
`nameTemplate`, `namespace`, `nodeId`, `notes`, `number`, `objective`, `outputByteLimit`, `outputLineLimit`
`owner`, `ownerKind`, `packageKind`, `packageName`, `packagePublish`, `path`, `paths`, `platformTags`
`plugins`, `policyGateIds`, `port`, `postgres`, `privateKeyPath`, `profile`, `profiles`, `projectId`
`projectKey`, `projectRoot`, `projects`, `projectsRoot`, `promotion`, `protocol`, `provenance`, `provider`
`providerFilters`, `providerIdentity`, `providerMutationAuthProfile`, `providerQuery`, `provisioning`, `publicRewrite`, `publication`, `purposes`
`push`, `pushUrl`, `queryLimit`, `reason`, `reasoning`, `relationships`, `relaunch`, `release`
`releasePublish`, `releaseTrain`, `remote`, `remoteUrl`, `remotes`, `repo`, `repositories`, `repository`
`repositoryId`, `repositoryName`, `repositoryOwner`, `repositoryScopes`, `repositoryUrl`, `requirePassing`, `required`, `requiredCapabilities`
`requiredChecks`, `requiredPermission`, `requiredProviderPermissions`, `retention`, `review`, `reviewBranchPattern`, `role`, `roleBindings`
...
`sshUser`, `staleAfterMs`, `staleChecks`, `staleClaimPolicy`, `start`, `statePath`, `statusFieldId`, `statusOptions`
`statuses`, `stopWhenNoEligibleWork`, `storePath`, `strategy`, `surfaces`, `tailscaleAddress`, `target`, `targetAgents`
`targetBranch`, `targetComponents`, `timeoutMs`, `title`, `tokenRefreshBufferSeconds`, `tools`, `trackerDiscovery`, `trackerId`
`trackerLimits`, `transport`, `trigger`, `trustSemantics`, `ttlDays`, `unknownActorFallbackRole`, `unscopedName`, `update`
`url`, `valueHint`, `variable`, `variant`, `verification`, `version`, `versionPlanning`, `versionPolicy`
`visibility`, `whileEligible`, `workItemClaims`, `workTrackerCommunication`, `workTrackers`, `workTracking`, `workspaceRoots`, `workspacesRoot`
`worktreesRoot`, `wrongBase`
